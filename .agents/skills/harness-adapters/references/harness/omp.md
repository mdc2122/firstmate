...
A secondmate registered with `remote=1` in `data/secondmates.md` is verified on omp as of 2026-09-16 (omp 18.1.20 on studio2): `../../../bin/fm-remote-secondmate-control.sh launch` and `relaunch` accept it, and the remote path runs the identical `../../../bin/fm-spawn.sh <id> <home> --secondmate` launch inside the `fm-remote` Herdr session on the remote host, so the verified launch contract above carries over unchanged; there is no `--remote` flag.
