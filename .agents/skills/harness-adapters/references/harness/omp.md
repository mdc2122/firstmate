...
| Fact | Value |
|---|---|
| Binary | `omp`, resolved from `PATH` by `../../../bin/fm-spawn.sh`; supported launcher shapes and their verification are linked under Detection below, and a missing executable refuses the spawn. |
...
| Marker | `OMPCODE=1` is OMP's child-process marker; `FM_OMP_HARNESS=omp` is Firstmate's launch marker; Detection below owns their precedence. |
...
`bin/fm-harness.sh` tests OMP's `OMPCODE=1` and Firstmate's ancestry-backed `FM_OMP_HARNESS=omp` before OMP's compatibility marker `CLAUDECODE=1`.
...
`bin/fm-session-lock-lib.sh` owns the anchored native-name and Bun-launcher identity rules shared by harness detection, session-lock ownership, and agent liveness classification.
See [runtime-backend verification](../../../../../docs/verification/runtime-backends.md#oh-my-pi-omp) for the version-scoped evidence and `tests/fm-omp-harness.test.sh` for regression coverage.
