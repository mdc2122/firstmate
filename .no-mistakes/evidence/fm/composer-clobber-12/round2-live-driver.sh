...
. "/Users/Morley/.no-mistakes/worktrees/62b3bc0be3e5/01M2NHCDJDC0NK3N7EF526H11P/tests/lib.sh"

ROOT="/Users/Morley/.no-mistakes/worktrees/62b3bc0be3e5/01M2NHCDJDC0NK3N7EF526H11P"
...
"$REAL_TMUX" -L "$SOCKET" new-session -d -x 120 -y 40 -s "$SESSION" -n control -c "$WORKSPACE" \
...
EV=/Users/Morley/.no-mistakes/evidence/01M2NHCDJDC0NK3N7EF526H11P
capture() {
 tmux capture-pane -p -t "$TARGET" > "$EV/round2-$1.txt"
 python3 - "$EV/round2-$1.txt" <<'PYHTML'
import sys,html,pathlib
p=pathlib.Path(sys.argv[1]); p.with_suffix('.html').write_text('<html><body style="background:#101820;color:#ddd"><pre>'+html.escape(p.read_text())+'</pre></body></html>')
PYHTML
}
capture multiline-cleared
start_run() {
 tmux send-keys -l -t "$TARGET" "$1"
 tmux send-keys -t "$TARGET" Enter
 for _ in $(seq 1 100); do
  [ "$(fm_busy_muse_run_state "$SESSION_LOG")" = busy ] && return 0
  sleep 0.1
 done
 fail 'run did not start'
}
start_run 'restore  repeated   spaces'
FM_HOME="$LAB/home" "$ROOT/bin/fm-send.sh" live --key Escape || fail 'single-line interrupt failed'
[ "$(fm_tmux_composer_state "$TARGET")" = empty ] || fail 'single-line restore remains'
capture single-line-cleared
pass 'Live restored repeated-space prompt cleared'
start_run 'active work for fresh-input guard'
tmux send-keys -l -t "$TARGET" 'captain fresh draft'
sleep 0.3
capture fresh-before
FM_HOME="$LAB/home" "$ROOT/bin/fm-send.sh" live --key Escape 2> "$EV/round2-fresh-warning.txt" || fail 'fresh interrupt failed'
content=$(fm_tmux_composer_content "$TARGET")
[ "$content" = 'captain fresh draft' ] || fail "fresh input changed: $content"
capture fresh-after
cat "$EV/round2-fresh-warning.txt"
pass 'Live fresh draft preserved'
tmux send-keys -t "$TARGET" C-c
sleep 0.3
start_run 'unprovable restored prompt'
rm "$LAB/home/state/live.muse-session"
FM_HOME="$LAB/home" "$ROOT/bin/fm-send.sh" live --key Escape 2> "$EV/round2-unprovable-warning.txt" || fail 'unprovable interrupt failed'
content=$(fm_tmux_composer_content "$TARGET")
[ "$content" = 'unprovable restored prompt' ] || fail "unprovable content changed: $content"
capture unprovable-after
cat "$EV/round2-unprovable-warning.txt"
pass 'Live unprovable restored prompt preserved with warning'
cleanup
trap - EXIT
