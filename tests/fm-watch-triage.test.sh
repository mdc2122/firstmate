...
# The liveness beacon is touched once at startup and then at the TOP of every
# poll, so this drops any beacon left by an earlier round, waits for THIS
# watcher to write a fresh one, then waits for the mtime to advance twice -
# the whole poll cycle between two consecutive loop-top touches is what the
# caller's assertions describe, and requiring two advances keeps that true
# even when the first observed touch was the startup beat rather than a
# poll-top touch.
# 0 if the watcher is still alive after a completed cycle, 1 if it exited.
wait_poll_cycle() {  # <state> <pid> [limit-ticks]
  local state=$1 pid=$2 limit=${3:-300} beat last now advances=0 i=0
